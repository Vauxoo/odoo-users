...
class merge_user_for_login(osv.Model):
    ...
    def change_gmail_fields(self, cr, uid, ids, context=None):
        '''
        Originally for do login with google, we use only 3 fields that allow to login with one
        gmail account. But now we can do login with multiple gmail accounts, for that, this method
        change the values in old fields and sets these in the new fields used to do login with
        ...
        for user_brw in user_obj.browse(cr, uid, user_ids, context=context):
            if not tokens_obj.search(cr, uid,
                                     [('oauth_provider_id', '=', user_brw.oauth_provider_id and \
                                      ...
                                      ('user_id', '=', user_brw.id)], context=context):
                user_brw.write({
                    'gmail_tokens':[(0, 0, {
                                            'name':user_brw.login,
                                            'oauth_provider_id': user_brw.oauth_provider_id and \
        ...
        for user_brw in user_obj.browse(cr, uid, user_ids, context=context):
            ...
            if users:
                ...
                action_id = self.create(cr, uid, {
                                                'access_token': token,
                                                'user_id': users[0] ,
                                                'user_ids': [(0, 0, {'user_id':i}) for i in users] ,
                                                })
    ...
    def do_pre_merge_from_users(self, cr, uid, ids, context=None):
        '''
        Send email to ask permission to do merge or do merge directly if the email of user to merge
        ...
        if wzr_brw.user_ids:
            if all([i.user_id.email == parent_brw.email and True or False for i in wzr_brw.user_ids]):
                ...
                context.update({'active_model':'res.users', 'active_ids': user_ids})
                fuse_obj.create(cr, SUPERUSER_ID, {}, context=context)
                wzr_brw.write({
                               'executed': True,
                              })
                cr.commit()
                context.update({'default_message':_('''Proccess finished users merged''')})
